...
test:
	...
	docker-compose ps
	docker run --net vnet -e HBASE_ZOOKEEPER_QUORUM=zookeeper-1.vnet:2181 --volumes-from regionserver-1 smizy/hbase:${VERSION}-alpine  bash -c 'for i in $$(seq 200); do nc -z regionserver-1.vnet 16020 && echo test starting && sleep 20; break; echo -n .; sleep 1; [ $$i -ge 200 ] && echo timeout && exit 124 ; done'
